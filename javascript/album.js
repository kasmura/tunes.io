...
function fenopyResult(torrentResults) {
  ...
  if(torrents.length > 0) {
      ...
      document.getElementById('dropdownTorrent').href = 'http://torcache.net/torrent/' + torrents[0]['hash'] + '.torrent';
...
function queryResult(data) {
  ...
  var fenopyUrl = "fenopy.php?keyword=" + encodeURI(album['artist'] + " " + album['name']) + "&callback=fenopyResult";
  runScript(fenopyUrl);
